...
import nibs_siamese_net as nib2
...
def testNibsNet2(data_lst, net, fn, num_iter):
    rho = 32
    patch_sz = 128
    height = 240
    width = 320
    ...
    net.eval()
    model_input = img_train.unsqueeze(0)
    out = net(model_input[:, 0:1, :, :], model_input[:, 1:2, :, :])
    ...
    if num_iter % 25 == 0:
        ...
        cv2.imwrite('./results2/{}_orig.tiff'.format(num_iter), img_cpy1)
        cv2.imwrite('./results2/{}_patch_orig.tiff'.format(num_iter), original_patch_c)
        cv2.imwrite('./results2/{}_wpd.tiff'.format(num_iter), img_wpd_cpy)
        cv2.imwrite('./results2/{}_patch_wpd.tiff'.format(num_iter), warped_patch_c)
...
def main():
    ...
    fn = open('./results2/results.txt', 'w')

    net = nib2.NibsNet2()
    cp = torch.load(dir_model2 + 'cp.tar')
    net.load_state_dict(cp['model_state_dict'])
    net.eval()
    ...
    for i in range(0, 1000):
        num_iter += 1
        err += testNibsNet2(data_lst, net, fn, num_iter)
